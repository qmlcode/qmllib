...
def tf_acsf(mols, path, elements):
    ...
    xyzs, zs = pad(
        max_atoms, [mol.coordinates for mol in mols], [mol.nuclear_charges for mol in mols]
    )
...
def fort_acsf_gradients(mols, path, elements):
    ...
    Xgrad_test = np.concatenate(
        [mol.gradients.reshape(mol.natoms**2, mol.gradients.shape[1] * 3) for mol in mols]
    )
