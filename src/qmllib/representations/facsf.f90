...
!   Introduced to make OpenMP parallelization easier.
   ...
   integer:: i, j, k, l, n, m, o, p, q, s, z, nelements, nbasis2, nbasis3, nabasis, j_id, max_j_id
   integer, allocatable, dimension(:) :: element_types, saved_j
...
!$OMP PARALLEL DO SCHEDULE(dynamic)
   do i = 1, natoms_tot
      do j = 1, nelements
      if (nuclear_charges(true_atom_id(i, natoms)) .eq. elements(j)) then
         element_types(i) = j
         exit
      end if
      end do
...
!$OMP PARALLEL DO PRIVATE(rij, n, rik, m, a, b, c, angle, cos_1, cos_2, cos_3,&
!$OMP radial, ksi3, angular, o, p, q, s, z) SCHEDULE(dynamic)
   do i = 1, natoms
   do j = 1, natoms_tot - 1
      if (i .eq. j) cycle
! distance between atoms i and j
